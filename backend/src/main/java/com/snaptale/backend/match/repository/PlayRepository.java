...
public interface PlayRepository extends JpaRepository<Play, Long> {
        ...
        // 특정 매치의 특정 플레이어가 특정 지역에 놓은 카드들의 파워 합계 조회
        @Query("SELECT COALESCE(SUM(p.powerSnapshot), 0) FROM Play p WHERE p.match.matchId = :matchId " +
                        "AND p.guestId = :guestId AND p.slotIndex = :slotIndex AND p.isTurnEnd = false")
